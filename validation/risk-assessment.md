...
| RA-19 | A recovery that fails partway leaves partial writes (half-filed order, orphan filing event, or a wrongly RESOLVED queue) | Med | High | **Med** | Each permitted operation is atomic. A handled `InboundError` rolls back the filing side effects and queue resolution, then commits the approved handled outcome: the attempted message `ERRORED`, a `FAILED` attempt, and the queue left `OPEN`. Separately, an unexpected non-`InboundError` rolls back the whole request and re-raises. Both leave no dangling transaction (R-038); filing is all-or-nothing. Tests: `test_handled_mid_operation_failure_rolls_back_all_side_effects`, `test_unexpected_failure_after_filing_rolls_back_entire_request`, file-backed durability, `PRAGMA foreign_key_check`. | Low |
...
3. **RA-17 / RA-18 / RA-19 (recovery integrity).** The v1.1 recovery service adds
   ...
   recovery per item, and every operation is atomic - a handled `InboundError`
   rolls back the filing side effects and commits the approved FAILED outcome
   (queue OPEN, message ERRORED), while an unexpected error rolls back the whole
   request and re-raises. These are enforced by schema constraints and the
   recovery service and are directly tested, including the two human-approved
